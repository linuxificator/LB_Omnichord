...
experimental native Windows x86_64 zip. Every package contains the Qt frontend
and compatible AMY runtime while preserving their separate-process
wire-protocol boundary. Windows uses a private named pipe; it does not run the
Linux AppImage through WSL.
...
Windows MIDI input and low-latency tuning remain outstanding.
