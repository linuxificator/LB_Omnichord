...
The portable application does not impose an operating-system scheduler policy.
For a dedicated instrument, an optional and reversible Pi 4 host profile has
now been physically measured: CPUs 0-1 handle the frontend, OS and IRQs, CPU 2
handles the two PipeWire data loops, and CPU 3 handles only AMY's detected
audio callback. See [`realtime_howto.md`](realtime_howto.md) before enabling it;
pinning the complete audio chain to one core was measured to be substantially
worse.
