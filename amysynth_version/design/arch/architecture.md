...
Delayed work has three explicit timing owners. AMY reusable-sequence commands
...
every `amy_start()`. Android's pinned AAR follows the same sequence. The current
ESP32-P4 release also embeds Gamma9001; its flash layout and resource profile
are validated by the dedicated firmware build and physical-device tests.
