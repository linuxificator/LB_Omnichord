...
class ProgramIntegrationTests(unittest.TestCase):
    ...
    def test_gated_strum_wraps_selected_timbre_and_can_be_disabled(self) -> None:
        with HeadlessApp(native_amy=False) as app:
            ...
            # Give the strum surface a remembered chord, then enable GTD.
            ...
            # Turning GTD off cancels its envelope generation and restores the
            # ordinary strum master level. A following tap must no longer force
            # the synth master to zero first.
            app.action("toggleStrumGate")
            self.assertFalse(bool(app.query("strumGateEnabled")))
