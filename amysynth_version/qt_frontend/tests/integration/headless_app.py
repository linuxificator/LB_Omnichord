...
def main() -> int:
    ...
    intonation_harm = omnichord.load_intonation_table(
        omnichord.MUSIC_DIR / "intonation_harm.json"
    )
    intonation_jv = omnichord.load_intonation_table(
        omnichord.MUSIC_DIR / "intonation_jv.json"
    )

    synth_index_by_key = {
        synth.key: index for index, synth in enumerate(synths)
    }
