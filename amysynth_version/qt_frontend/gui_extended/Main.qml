...
Base.Main {
    ...
    readonly property real gateContentX:
        8 + Math.max(0, (gateViewportWidth - (strumX + strumWidth) * gateScale) / 2)
    readonly property real gateContentY:
        8 + Math.max(0, (gateViewportHeight - totalControlHeight * gateScale) / 2)

    Item {
        id: titleOverlay
        x: rootWindow.gateContentX + 520 * rootWindow.gateScale
        y: rootWindow.gateContentY
        width: (rootWindow.strumX + rootWindow.strumWidth - 520) * rootWindow.gateScale
            ...
            Rectangle {
                ...
                Button {
                    ...
                    background: Rectangle {
                        radius: width / 2
                        color: backend.strumGateEnabled ? "#2474b8" : "#a9c9e2"
                        border.color: "#15598f"
                        border.width: 2
                    }
                    contentItem: Text {
                        text: gatedButton.text
                        color: backend.strumGateEnabled ? "#ffffff" : "#173b58"
